...
void LatticeRestClient::SetEntityId(std::string entity_id) {
  if (!entity_id.empty()) {
...
bool LatticeRestClient::FetchToken() {
  if (client_id_.empty() || client_secret_.empty() || env_token_.empty() ||
      host_.empty()) {
    std::cerr << "[mlb] FetchToken: missing credential or endpoint\n";
    return false;
  }
  ...
  if (!SplitHostPort(host_, &oauth_host, &oauth_port) || oauth_host.empty()) {
    std::cerr << "[mlb] FetchToken: bad endpoint\n";
    return false;
  }
  ...
  std::cerr << "[mlb] FetchToken: URL=https://" << oauth_host
            << "/api/v1/oauth/token\n";

  HINTERNET hSession =
      WinHttpOpen(L"mavlink-lattice-bridge/0.1", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                  WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
  if (!hSession) {
    std::cerr << "[mlb] FetchToken: WinHttpOpen failed gle=" << GetLastError()
              << "\n";
    return false;
  }
...
#define WINHTTP_FLAG_SECURE_PROTOCOL_TLS1_3 0x00002000
  ...
  if (!hConnect) {
    std::cerr << "[mlb] FetchToken: WinHttpConnect failed gle=" << GetLastError()
              << "\n";
  ...
  if (!hRequest) {
    std::cerr << "[mlb] FetchToken: WinHttpOpenRequest failed\n";
  ...
  if (!WinHttpAddRequestHeaders(hRequest, wExtra.c_str(),
    ...
    std::cerr << "[mlb] FetchToken: WinHttpAddRequestHeaders failed\n";
  ...
  if (!sent || !WinHttpReceiveResponse(hRequest, nullptr)) {
    std::cerr << "[mlb] FetchToken: HTTP request failed gle=" << GetLastError()
              << "\n";
    WinHttpCloseHandle(hRequest);
    WinHttpCloseHandle(hConnect);
    WinHttpCloseHandle(hSession);
  ...
  if (!WinHttpQueryHeaders(hRequest,
    ...
    std::cerr << "[mlb] FetchToken: status query failed\n";
    WinHttpCloseHandle(hRequest);
    WinHttpCloseHandle(hConnect);
    WinHttpCloseHandle(hSession);
    return false;
  ...
  std::string resp;
  if (!ReadHttpResponseBody(hRequest, &resp)) {
    std::cerr << "[mlb] FetchToken: read body failed\n";
    WinHttpCloseHandle(hRequest);
    WinHttpCloseHandle(hConnect);
    WinHttpCloseHandle(hSession);
    return false;
  ...
  std::string resp_log = resp;
  if (resp_log.size() > 240) {
    resp_log.resize(240);
    resp_log += "...";
  }
  std::cerr << "[mlb] FetchToken: HTTP " << status << " body=" << resp_log
            << "\n";
  ...
  if (!ExtractJsonStringField(resp, "access_token", &token)) {
    std::cerr << "[mlb] FetchToken: no access_token in response\n";
    return false;
  }
  ...
  std::cerr << "[mlb] FetchToken: access_token acquired expires_in="
            << expires_in << "s\n";
  return true;
}
...
bool LatticeRestClient::EnsureToken() {
  ...
  // Sandbox fallback used by HOTL when /oauth/token is unavailable.
  if (env_token_.empty()) {
    return false;
  }
  access_token_ = env_token_;
  token_expiry_ = std::chrono::steady_clock::now() + std::chrono::hours(1);
  std::cerr << "[mlb] EnsureToken: OAuth unavailable - using sandbox JWT as "
               "Bearer\n";
  return true;
}

std::string LatticeRestClient::BuildEntityJson(const VehiclePose& pose) const {
  const std::time_t now = std::time(nullptr);
  const std::string created = Rfc3339Utc(now);
  const std::string expiry = Rfc3339Utc(now + 120);

  char lat[64], lon[64], alt[64];
  std::snprintf(lat, sizeof(lat), "%.8f", pose.lat_deg);
  std::snprintf(lon, sizeof(lon), "%.8f", pose.lon_deg);
  std::snprintf(alt, sizeof(alt), "%.3f", pose.alt_m);

  std::ostringstream o;
  o << "{"
    << "\"entityId\":\"" << JsonEscape(entity_id_) << "\","
...
PublishResult LatticeRestClient::PublishPose(const VehiclePose& pose) {
  PublishResult result;
  if (!pose.valid) {
    result.body = "pose is not valid";
    return result;
  }
  if (!EnsureToken()) {
  ...
  const std::string body = BuildEntityJson(pose);
...
#define WINHTTP_FLAG_SECURE_PROTOCOL_TLS1_3 0x00002000
  ...
  // Proven HOTL path: PUT /api/v1/entities with Entity JSON (entityId in body).
  HINTERNET hRequest = WinHttpOpenRequest(
...
VehiclePose MakeSimPose() {
  ...
  constexpr double lat0 = 32.7767;
  constexpr double lon0 = -96.7970;
  const double ang = std::fmod(t, 60.0) / 60.0 * 2.0 * 3.14159265358979323846;
  const double dlat = (200.0 / 111320.0) * std::cos(ang);
  const double dlon =
      (200.0 / (111320.0 * std::cos(lat0 * 3.14159265358979323846 / 180.0))) *
      std::sin(ang);
  VehiclePose pose;
  pose.lat_deg = lat0 + dlat;
  pose.lon_deg = lon0 + dlon;
  pose.alt_m = 120.0;
  pose.heading_deg = std::fmod(ang * 180.0 / 3.14159265358979323846 + 90.0, 360.0);
  pose.speed_mps = 15.0;
  pose.valid = true;
  pose.source = "sim";
  return pose;
